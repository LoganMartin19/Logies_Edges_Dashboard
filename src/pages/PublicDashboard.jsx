...
const TeamChip = ({ name, align = "left" }) => (
  <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
    {align === "left" && (
      <img loading="lazy" src={logoUrl(name)} alt="" width={18} height={18} onError={(e) => (e.currentTarget.style.display = "none")} />
    )}
          ...
          <div style={{ marginTop: 8 }}>
            {(t.legs || []).map((l, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 8, padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,.08)" }}>
                ...
                <div style={{ textAlign: "right" }}>{marketLabel(l.market)}</div>
                <div style={{ textAlign: "right" }}>
                  {priceDisplay(l.price)} {l.bookmaker ? `(${l.bookmaker})` : ""}
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.headerRow}>
        ...
        <label>
          Day:&nbsp;<input style={S.input} type="date" value={day} onChange={(e) => setDay(e.target.value)} />
        </label>
      ...
      <div style={S.picksWrap}>
        ...
        {picks.length === 0 ? (
          <div style={{ background: "rgba(255,255,255,.08)", borderRadius: 12, padding: 12, color: "#d7e6db" }}>No featured picks yet.</div>
        ) : (
          picks.map((p, i) => (
            <Link key={i} to={routeFor({ id: p.fixture_id ?? p.id, sport: p.sport })} style={{ textDecoration: "none", color: "inherit" }}>
              <div style={S.pickRow}>
                <div style={S.pickTeam}>
                  <img src={logoUrl(p.home_team)} width={20} alt="" onError={(e) => (e.currentTarget.style.display = "none")} /> {p.home_team}
                </div>
                ...
                <div style={{ ...S.pickTeam, justifyContent: "flex-end" }}>
                  {p.away_team} <img src={logoUrl(p.away_team)} width={20} alt="" onError={(e) => (e.currentTarget.style.display = "none")} />
                </div>
                <div style={{ gridColumn: "1 / -1", ...S.pickSub }}>
                  {marketLabel(p.market)} {priceDisplay(p.price)} — {p.bookmaker}
