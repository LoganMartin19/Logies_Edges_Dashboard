...
const COMP_NAMES = {
  ...
  LG1: "EFL League One",
  LG2: "EFL League Two",
  NAT_LEAGUE: "National League",
  NAT_NORTH: "National League North",
  NAT_SOUTH: "National League South",
  ENG_FA: "FA Cup",
  ENG_EFL: "EFL Cup",
...
export default function Performance() {
  ...
  return (
    ...
    >
      ...
      {/* Premium upsell – shows gold "See Premium" button for non-premium users */}
      <PremiumUpsellBanner
        mode="link"
        message="See deeper breakdowns, longer history and premium-only stats with CSB Premium."
      />
      ...
      {loading ? (
        ...
        <>
          ...
          >
            ...
            <Kpi
              title="Record"
              value={`${kpis.record.won}W-${kpis.record.lost}L-${kpis.record.void}V`}
            />
            <Kpi
              title="Staked"
              value={`£${kpis.staked.toFixed(2)}`}
            />
            <Kpi
              title="Returned"
              value={`£${kpis.returned.toFixed(2)}`}
            />
            <Kpi
              title="P/L"
                ...
                <tbody>
                  {byMarket.map((m) => (
                    <tr
                      key={m.market}
                      style={{
                        borderTop: `1px solid ${COL.cardBorder}`,
                      }}
                    >
                      <Td>{m.market}</Td>
          ...
          <Section title="By league / competition">
            {!byLeague.length ? (
              <p style={{ color: COL.muted }}>
                No picks for this period.
              </p>
            ) : (
              <table
                style={{ width: "100%", borderCollapse: "collapse" }}
